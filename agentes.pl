...
todo_limpio([]).
todo_limpio([(X1, _, _, _, _)|Y]):-X1 == 0, todo_limpio(Y).

contar_limpias([], 0).
contar_limpias([(X1, _, _, _, _)|Y], Z):-X1 == 0, contar_limpias(Y, C), Z is C+1.
contar_limpias([(X1, _, _, _, _)|Y], Z):-X1 == 1, contar_limpias(Y, C), Z is C.

contar_sucias([], 0).
contar_sucias([(X1, _, _, _, _)|Y], Z):-X1 == 1, contar_sucias(Y, C), Z is C+1.
contar_sucias([(X1, _, _, _, _)|Y], Z):-X1 == 0, contar_sucias(Y, C), Z is C.

contar_vacias([], 0).
contar_vacias([(_, X2, X3, _, _)|Y], Z):-X2 == 0, X3 == 0, contar_vacias(Y, C), Z is C+1.
contar_vacias([(_, X2, X3, _, _)|Y], Z):-(X2 == 1; X3 == 1), contar_vacias(Y, C), Z is C.

todos_en_guarderias([]).
todos_en_guarderias([(_, _, X3, X4, _)|Y]):-X4 == 1, !, X3 == 1, !, todos_en_guarderias(Y).
todos_en_guarderias([(_, _, _, X4, _)|Y]):-X4 == 0, todos_en_guarderias(Y).

pasa_el_60(X):-contar_vacias(X, C), contar_sucias(X, R), R > ((C/100)*60).

final_state([]).
final_state(X):-todo_limpio(X), !, todos_en_guarderias(X).
final_state(X):-pasa_el_60(X).
